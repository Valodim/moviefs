...
config = {}
config['apikey'] = "6b5acae77886d2b07246057163e2920b"
...
stopwords = [ 'mkv', 'german', '720p', '1080p', 'hdtv', 'ac3', 'bluray', 'dts', 'h264', 'x264', 'rip', '196', '197', '198', '199', '200', '201', '(', '[' ]
def movieinfo(fname, guessname):
    miout = subprocess.check_output([ "midentify", fname ])
    attrs = { }
    for line in miout.split("\n"):
        if line:
    ...
    return {
        'fname': fname,
        'guessname': guessname,
        'movie': search(guessname.decode('utf-8')),
        'attrs': attrs,
    }

path_base = '/home/shared/v/'
path_hd = 'hd/'
path_sd = 'movies/'
...
def moviepath(info):
    ...
    if int(info['attrs']['ID_VIDEO_WIDTH']) >= 1280 or int(info['attrs']['ID_VIDEO_HEIGHT']) >= 720:
        path += path_hd
    else:
        path += path_sd

    path += info['movie']['name'].replace(os.sep, ' ')

    i = 0
    langs = []
    while ('ID_AID_' + str(i) + '_LANG') in info['attrs']:
        if info['attrs']['ID_AID_' + str(i) + '_LANG'] == 'deu':
            langs.append('ger')
        else:
            langs.append(info['attrs']['ID_AID_' + str(i) + '_LANG'])
        i += 1
    ...
    if len(langs) > 0:
        ...
        print "No language info found."

    path += '/'

    return path

def findmovieinfo(fname):
    guessname = fname
    guessname = os.path.basename(fname).lower()
    guessname = guessname.replace('.', ' ').replace('-', ' ').replace('_',' ')
    for s in stopwords:
        if guessname.find(s):
    ...
    while True:
        ...
        if info is None or len(info['movie']) == 0:
            ...
            guessname = raw_input("Input Title (or empty to skip): ")
            if guessname != '':
                continue
            return None

        if len(info['movie']) == 1:
            info['movie'] = info['movie'][0]
            return info
        else:
            print "More than one title candidates found! Please select:"
            for i in range(0, len(info['movie'])):
                print " ", i+1, "-", unicode(info['movie'][i])
            selection = 0
            while selection == 0:
                selection = raw_input("Select by number (1-" + str(len(info['movie'])) + "), (e)dit or (s)kip: ")
                if selection == 'e':
                    guessname = ''
                    break
                if selection == 's':
                ...
                if selection > 0 and selection <= len(info['movie']):
                    info['movie'] = info['movie'][selection-1]
                    return info
                else:
                    selection = 0
...
def main():
    ...
    for fname in sys.argv[1:]:
        ...
        while True:
            ...
            if verbose:
                print "video file attributes: "
                for key in info['attrs']:
                    print " -", key, "=", info['attrs'][key]
            else:
            ...
            ack = raw_input("Shall I move it? (Y/n/e) ")
            if ack != 'e':
                break

        if ack is not None and (ack == 'Y' or ack == 'y' or ack == ''):
            ...
            if not os.access(path, os.W_OK):
                # error!
                print "error, destination path not writable!"
                continue

            # if it's not writable
            if os.access(path + os.path.basename(info['fname']), os.F_OK):
                # error!
                print "error, destination filename already exists!"
                continue

            print "mv: ", info['fname'], "->", path + os.path.basename(info['fname'])
            if not dry:
                os.rename(info['fname'], path + os.path.basename(info['fname']))
            print "Done!"
